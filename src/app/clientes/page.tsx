    ...
    <div className="min-h-screen bg-gray-950 pb-24">
      <div className="bg-gray-900 px-4 pt-12 pb-4 border-b border-gray-800">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => window.location.href = '/clientes/nuevo'} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-semibold">+ Nuevo</button>
        </div>
        <input placeholder="Buscar cliente..." value={busqueda} onChange={e => setBusqueda(e.target.value)} className="w-full bg-gray-800 text-white rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500 text-sm mb-3" />
          ...
          <div className="space-y-2">
            {clientesFiltrados.map(c => (
              <div key={c.id} onClick={() => window.location.href = `/clientes/${c.id}`} className="bg-gray-900 rounded-xl p-4 border border-gray-800 flex items-center justify-between cursor-pointer hover:border-blue-500 transition-all">
                ...
                <span className={`text-xs px-2 py-1 rounded-lg font-medium ${c.estado === 'vendido' ? 'bg-green-900 text-green-400' : c.estado === 'perdido' ? 'bg-red-900 text-red-400' : 'bg-gray-800 text-gray-400'}`}>{c.estado}</span>
              </div>
            ))}
          </div>
        )}
